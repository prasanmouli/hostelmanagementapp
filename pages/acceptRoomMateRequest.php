...
$x=0;
$no;
$approvalId = mysql_real_escape_string($_GET['approvalId']);
$q= " SELECT capacity FROM hostels,userPreferences WHERE hostels.commonName = userPreferences.hostelName AND userPreferences.userId = ".$userid;
$q1=mysql_query($q);
...
while($info=mysql_fetch_array($res)){
	...
	if($info['accepted']!='0'&$info['accepted']!='1'){
	...
	$res3=mysql_query($query3);
	  $x++;
		...
		$checkDet = mysql_fetch_array(mysql_query($checkReq));
	  	for($i=1;$i<$capacity;$i++){
		  if(!$checkDet['roomMateId'.$i]){
		$no=$i;
		echo $no;
		}
		}
		if($no!=0&&$no<$capacity)
		$query4="UPDATE finalRoomList SET roomMateId".($no+1)."= ".$userid;
		else if($capacity==$no)
		$data = "Bigerror";
		else
